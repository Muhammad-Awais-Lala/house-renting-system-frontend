...
import { ChevronLeft, MapPin, Maximize, Home, MessageSquare, Calendar, CheckCircle, AlertCircle, Info, Map as MapIcon, Star, X } from 'lucide-react';
...
export default function PropertyDetailsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const propertyResponse = await propertyService.getById(id);
        setProperty(propertyResponse.data);
        
        // Fetch reviews for this property
        try {
          const reviewsResponse = await reviewService.getPropertyReviews(id as string);
  ...
  if (!property) return <div className="p-12 text-center text-red-600">Property not found.</div>;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-4">
           <div className="aspect-[16/10] rounded-3xl overflow-hidden shadow-2xl shadow-indigo-100 border border-white">
              <img src={images[0]} alt={property.title} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
           </div>
        ...
        <div className="space-y-8">
           <div>
              ...
              <div className="flex flex-wrap items-center gap-6 text-slate-500">
                 ...
                 <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm font-medium">{property.propertySize} {property.sizeUnit}</span>
                 </div>
                 {property.averageRating && (
                   <div className="flex items-center gap-1.5">
                      <div className="flex items-center gap-0.5">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} className={`h-4 w-4 ${i < Math.round(property.averageRating) ? 'fill-yellow-400 text-yellow-400' : 'text-slate-300'}`} />
                        ))}
                      </div>
           ...
           <div className="p-8 rounded-3xl bg-slate-900 text-white shadow-2xl shadow-indigo-100 relative overflow-hidden">
              ...
              <div className="mt-8 grid grid-cols-2 gap-4 relative z-10">
                 <Button 
                   className="rounded-2xl py-4 bg-white text-slate-900 hover:bg-slate-100 font-black h-14" 
                   onClick={() => {
                 ...
                 >
                    {isBooked ? (
                       <><CheckCircle className="mr-2 h-5 w-5" /> REQUESTED</>
                    ) : (
                       <><Calendar className="mr-2 h-5 w-5" /> BOOK VISIT</>
                    )}
                 </Button>
              </div>
           </div>
           ...
           <div className="space-y-4">
              ...
              <p className="text-slate-600 leading-relaxed text-lg italic bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
             ...
             <div>
                ...
                <div className="flex flex-wrap gap-2">
                   {property.amenities.map((amenity: string, idx: number) => (
                      <span key={idx} className="px-4 py-2 bg-indigo-50 text-indigo-700 text-sm font-bold rounded-xl border border-indigo-100">
                         {amenity}
                      </span>
                   ))}
                </div>
              ...
              <div key={review._id} className="p-6 border border-slate-200 rounded-2xl">
                <div className="flex items-start justify-between mb-3">
                  ...
                  {review.isVerifiedTenant && (
                    <span className="px-2 py-1 bg-green-50 text-green-700 text-xs font-bold rounded">Verified</span>
                  )}
                </div>
