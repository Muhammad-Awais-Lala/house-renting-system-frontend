                ...
                >
                  <div className="h-12 w-12 rounded-full bg-slate-100 flex items-center justify-center border border-slate-200 overflow-hidden shrink-0">
                    {otherPerson?.profileImage ? (
                       <img src={otherPerson.profileImage} alt={otherPerson.firstName} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 text-left overflow-hidden">
                    <p className="font-bold text-slate-900 truncate">
                      {otherPerson?.firstName ? `${otherPerson.firstName} ${otherPerson.lastName}` : 'User'}
                    </p>
          ...
          <>
              ...
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <h3 className="font-black text-slate-900">
                      {getOtherParticipant(activeChat)?.firstName ? `${getOtherParticipant(activeChat)?.firstName} ${getOtherParticipant(activeChat)?.lastName}` : 'User'}
                    </h3>
            ...
            >
              ...
              {messages.map((msg: any, idx: number) => {
                const isMe = msg.senderId === user?._id || msg.senderId === (user as any)?.id;
                return (
                  <div key={msg._id || idx} className={cn("flex w-full", isMe ? "justify-end" : "justify-start")}>
                    <div className={cn(
                      "max-w-[75%] px-5 py-3 rounded-3xl text-sm shadow-sm",
                      isMe 
                      ...
                      <span className={cn(
                        "text-[10px] mt-1.5 block opacity-70 font-semibold",
